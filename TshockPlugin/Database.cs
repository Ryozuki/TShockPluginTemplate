    ...
    public class Database
    {
        ...
        public static void Connect()
        {
            switch (TShock.Config.StorageType.ToLower())
            {
                ...
                case "sqlite":
                    string sql = Path.Combine(TShock.SavePath, "$safeprojectname$.sqlite");
            ...
            sqlcreator.EnsureTableStructure(new SqlTable("$safeprojectname$",
        ...
        public static void addSomething(int something)
        {
            try
            {
                db.Query("INSERT INTO $safeprojectname$ (Name) VALUES (@0)",
                    something
                );
            }
            catch (Exception e)
        ...
        public static void delSomething(int something)
        {
            try
            {
                db.Query("DELETE FROM $safeprojectname$ WHERE UserID = @0", something);
            }
            catch (Exception e)
            {
                Console.WriteLine(e.StackTrace);
